...
exports.OrderPage = class OrderPage {
  ...
  async selectOrderByOrderFrom(expectedOrderFrom) {
    ...
    console.log(
      JSON.stringify({ searching: expectedOrderFrom, totalRows: rowCount }),
    );
    ...
    for (let i = 0; i < (await rows.count()); i++) {
      ...
      if (orderId.some((id) => text.includes(id))) {
        ...
        const viewButton = row.locator(
          "button[aria-label='View Incoming Order Details']",
        );
  ...
  async getTableData(orderIds = []) {
    ...
    const headers = await this.page
      .locator("#wrapper-content thead th")
      .allTextContents();
    ...
    for (let i = 0; i < (await rows.count()); i++) {
      ...
      if (
        orderIds.length === 0 ||
        orderIds.some((id) => rowText.includes(id))
      ) {
        const cells = row.locator("td");
  ...
  async getFullProductTableData() {
    ...
    await this.page.waitForFunction(
      () => {
        ...
        const tfootRows = document.querySelectorAll(
          "#wrapper-content tfoot tr",
        );
    ...
    const result = await this.page.evaluate(() => {
      const toFloat = (str) =>
        parseFloat((str ?? "").trim().replace(/,/g, "")) || 0;
      const toInt = (str) => parseInt((str ?? "").trim()) || 0;
      const text = (el) => el?.textContent?.trim() ?? "";

      // ── tbody ────────────────────────────────────────────────────────────
      const rows = Array.from(
        document.querySelectorAll("#wrapper-content tbody tr"),
      );
      const products = rows
        .map((row) => {
      ...
      // ── tfoot ────────────────────────────────────────────────────────────
      const tfootRows = Array.from(
        document.querySelectorAll("#wrapper-content tfoot tr"),
      );
      ...
      console.log(
        "r0 cells:",
        r0.map((td) => td.textContent.trim()),
      );
      console.log(
        "r3 cells:",
        r3.map((td) => td.textContent.trim()),
      );
    ...
    try {
      ...
      for (let i = 0; i < rowCount; i++) {
        ...
        if (matching?.quantity !== undefined) {
          ...
          console.log(
            `✅ Updated → ${currentCode} : ${matching.quantity} (verified in DOM: ${written})`,
          );
      ...
      if (testData.grossDiscount !== undefined) {
        const gdInput = this.page.locator(
          'input[placeholder*="Gross Discount"]',
        );
      ...
      console.log(
        `✅ Total fields updated: ${verifiedProducts.length}. Clicking Update Button...`,
      );
      ...
      try {
        await Promise.race([
          this.page.waitForURL((url) => url.toString() !== urlBeforeSave, {
            timeout: 15000,
          }),
