...
test("TC_05_SND_Order_ApprovalPendingOrderFilterTest", async ({ page }) => {
  // Login Page
...
const count = await orderPage.getSelectedItemCount();
console.log(`Items selected: ${count}`);
await expect(count).toBe(appPendingData.orderId.length);
});
