...
test("SND_PI_CreateWrongSearchFilterTest", async ({ page }) => {
  // Login Page
  const loginPage = new LoginPage(page);
  const data = loginData[0];
  await loginPage.gotoLoginPage(data.url);
  ...
  const found = await createPIPage.selectOrderByOrderFrom(
    piData.retailerNameInvalid,
  );
  expect(
    found,
    `Retailer Name: "${piData.retailerNameInvalid}" not found in table`,
  ).toBe(false);
  console.log(
    `Retailer Name: "${piData.retailerNameInvalid}" not found in table`,
  );
