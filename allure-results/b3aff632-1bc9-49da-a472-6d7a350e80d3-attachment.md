...
```
ReferenceError: netPayable is not defined
```
...
```ts
  ...
  38 |   const value = await orderPage.getNetPayableValue();
  39 | console.log(value);
> 40 | expect(netPayable).not.toBeNull();
     |        ^ ReferenceError: netPayable is not defined
  41 | expect(netPayable).not.toBe("");
  42 | });
  43 | 
```