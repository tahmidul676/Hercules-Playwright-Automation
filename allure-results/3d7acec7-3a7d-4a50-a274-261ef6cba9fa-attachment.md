...
Expected substring: "One Thousand Three Hundred Eighty"
Received string:    "Two Thousand Five Hundred Forty"
```
...
```ts
  ...
  57 | console.log("Products:", tableData.products);
  58 | console.log("Summary:", tableData.summary);
  59 | 
  60 | // ── Row count ────────────────────────────────────────────────────────────────
  61 | expect(tableData.products).toHaveLength(3);
  62 | 
  63 | // ── Summary ──────────────────────────────────────────────────────────────────
  64 | expect(tableData.summary.netPayable).toBe(2540);
> 65 | expect(tableData.summary.netPayableInWords).toContain(
     |                                             ^ Error: expect(received).toContain(expected) // indexOf
  66 |   "One Thousand Three Hundred Eighty"
  67 | );
  68 | 
  69 | // ── Individual items ─────────────────────────────────────────────────────────
  70 | const [firstItem, secondItem, thirdItem] = tableData.products;
  71 | 
  72 | // ❌ REMOVED: you can't expect the same field to equal two different values
  73 | // expect(firstItem.code).toBe("SKC01");      // was conflicting
  74 | // expect(firstItem.code).toBe("SKU00013");   // with this
  75 | 
  76 | expect(firstItem.code).toBe("SKC01");       // row 0
  77 | expect(secondItem.code).toBe("SKU00013");   // row 1
  78 | expect(thirdItem.code).toBe("SKU00017");    // row 2
  79 | 
  80 | // Quantities come from approvalPendingOrder.json (all set to 3)
  81 | expect(firstItem.quantity).toBe(3);
  82 | expect(secondItem.quantity).toBe(3);
  83 | expect(thirdItem.quantity).toBe(3);
  84 | 
  85 | });
  86 | 
```