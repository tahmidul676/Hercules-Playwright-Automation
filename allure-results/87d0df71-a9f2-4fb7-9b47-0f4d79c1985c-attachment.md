...
Call log:
  - waiting for locator('#wrapper-content tbody tr').first().locator('td:nth-child(1) div')
...
```ts
  ...
  201 | 
  202 | 
  203 | async getProductTableData(orderIds = []) {
  204 | 
  205 |   const scrollContainer = this.page.locator(".table-responsive.report-table");
  206 | 
  207 |   // 🔥 scroll to load full table
  208 |   await scrollContainer.evaluate(el => {
  209 |     el.scrollTop = 0;
  210 |   });
  211 | 
  212 |   await this.page.waitForTimeout(500);
  213 | 
  214 |   const rows = this.page.locator("#wrapper-content tbody tr");
  215 | 
  216 |   const data = [];
  217 | 
  218 |   for (let i = 0; i < await rows.count(); i++) {
  219 | 
  220 |     const row = rows.nth(i);
  221 | 
  222 |     // ensure row is in view
  223 |     await row.scrollIntoViewIfNeeded();
  224 | 
  225 |     const rowText = await row.textContent();
  226 | 
  227 |     if (orderIds.length === 0 || orderIds.some(id => rowText.includes(id))) {
  228 | 
> 229 |       const code = await row.locator("td:nth-child(1) div").textContent();
      |                                                             ^ Error: locator.textContent: Test timeout of 60000ms exceeded.
  230 |       const description = await row.locator("td:nth-child(2) div").textContent();
  231 |       const quantity = await row.locator("td:nth-child(3)").textContent();
  232 | 
  233 |       data.push({
  234 |         code: code?.trim(),
  235 |         description: description?.trim(),
  236 |         quantity: quantity?.trim()
  237 |       });
  238 |     }
  239 |   }
  240 | 
