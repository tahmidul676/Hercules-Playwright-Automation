...
```
Error: locator.isDisabled: Error: strict mode violation: locator('ul.pagination button').filter({ hasText: '>' }) resolved to 2 elements:
    1) <button class="page-link  shadow-none">></button> aka getByRole('button', { name: '>', exact: true })
    2) <button disabled class="page-link  shadow-none">>></button> aka getByRole('button', { name: '>>' })

Call log:
  - waiting for locator('ul.pagination button').filter({ hasText: '>' })
