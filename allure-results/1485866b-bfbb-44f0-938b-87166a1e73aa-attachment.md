...
```ts
  ...
  123 | 
  124 |     const hasCheckbox = await row.locator("input[type='checkbox']").count();
  125 |     if (hasCheckbox === 0) continue;
  126 | 
  127 |     if (cellText.includes(expectedOrderFrom)) {
  128 |       const checkbox = row.locator("input[type='checkbox']");
  129 | 
  130 |       // Check disabled state for debugging
> 131 |       const isDisabled = await checkbox.isDisabled();
      |                                         ^ Error: locator.isDisabled: Test timeout of 60000ms exceeded.
  132 |       console.log(`Checkbox disabled: ${isDisabled}`);
  133 | 
  134 |       // Use JS click — bypasses disabled, visibility, and overlap
  135 |       await checkbox.evaluate(el => el.click());
  136 | 
  137 |       found = true;
  138 |       break;
  139 |     }
